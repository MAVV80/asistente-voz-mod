...
def procesar_comando(comando_normalizado: str, generador_voz: GeneradorVoz) -> None:
    ...
    if tokens[0] not in WAKE_WORDS:
        ...
        print(f"(Falta andy al inicio. Transcripción: {muestra})")
        generador_voz.generar_voz(
            "Inicia el comando con andy."
        )
        return
    ...
    # Fútbol - Últimos resultados
        ...
        ejecutar_ultimos_resultados(" ".join(cola), hablar)
...
def main() -> None:
    ...
    try:
        ...
        print("\nSaliendo.")
        generador.generar_voz("Hasta luego Manuel Andrés.")
